...
for (var i = BOOKMARKS.chapters.length - 1; i >= 0; i--) {
    ...
    bookMarkObj[chapterItem.chapterUid].marks = BOOKMARKS.updated.filter(mark => mark.chapterUid === chapterItem.chapterUid);
}
for (var i = REVIEWS.reviews.length - 1; i >= 0; i--) {
    let reviewItem = REVIEWS.reviews[i].review;
    bookMarkObj[reviewItem.chapterUid] = bookMarkObj[reviewItem.chapterUid] || Object.assign({
        marks: []
    })
    let sameMarksItem = bookMarkObj[reviewItem.chapterUid].marks.find(marksItem => reviewItem.range === marksItem.range);
    if (sameMarksItem) {
...
// 通过 range 来排序标注/评论
// console.log(JSON.stringify(bookMarkObj))
let markdownText = '';
